...
export default class AdminBadgesShowRoute extends Route {
  ...
  model(params) {
    ...
    return this.modelFor("adminBadgeExtension").findBy(
      "id",
  ...
  @action
  editGroupings() {
    const model = this.controllerFor("admin-badge-extension").get("badgeGroupings");
  ...
  updateGroupings(groupings) {
    this.controllerFor("admin-badge-extension").set("badgeGroupings", groupings);
  }
